...
class DataGenerator_3output_train(keras.utils.Sequence):
    ...
    def __data_generation(self, train_temp, labels_temp):
        ...
        y_tuple = tuple(tf.convert_to_tensor(labels_temp[key].values, dtype=tf.float32) for key in labels_temp.keys())

        return tf.convert_to_tensor(train_temp, dtype=tf.float32), y_tuple
